@extends('layouts.app')

@section('content')
    <x-common.page-breadcrumb pageTitle="Content visi" />
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-2">

        @if (session()->has('success'))
            <x-alert message="{{ session('success') }}" />
        @endif

        <div class="mt-6 justify-start">
            {{-- <h2 class="font-semibold text-xl">Content visi</h2> --}}
            <a href="{{ route('visi.create_visi') }}">
                <button class="bg-blue-500 text-white px-10 py-2 rounded-md font-semibold">Add</button>
            </a>

        </div>

        <div class="grid md:grid-cols-3 grid-cols-1 mt-4 gap-6">
            @foreach ($visis as $visi)
                <div>
                    {{-- <img src="{{ asset('storage/public/' . $visi->foto) }}" class="h-auto w-full object-cover" /> --}}
                    <div class="flex flex-col max-w-lg p-6 mx-auto text-gray-900 bg-white border border-gray-100 rounded-lg shadow dark:border-gray-600 xl:p-8 dark:bg-gray-800 dark:text-white">
                        <label class="mt-5 font-bold" for="visi">Visi</label>
                        <p class="text-xl font-light">{{ $visi->title }}</p>
                        <label class="mt-5 font-bold" for="deskripsi visi">deskripsi visi</label>
                        <p class="font-semibold text-gray-400">{{ $visi->deskripsi }}</p>
                    </div>
                    <a href="{{ route('visi.edit_visi', $visi) }}">
                        <button class="bg-gray-100 px-10 py-2 w-full rounded-md font-semibold">Edit</button>
                    </a>
                </div>
            @endforeach
        </div>


        <div class="mt-4">
            {{ $visis->links() }}
        </div>

    </div>

@endsection
